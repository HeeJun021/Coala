...
from sqlalchemy import func, desc, literal, select, union_all, cast
...
def get_dashboard_summary(db: Session) -> dict:
    ...
    today_post_reports = (
        ...
        .filter(func.date(PostReport.created_at) == today)
        .scalar()
    )
    today_comment_reports = (
        db.query(func.count(CommentReport.report_id))
        .filter(func.date(CommentReport.created_at) == today)
        .scalar()
    )
...
def get_recent_reports(db: Session, limit: int = 10):
    ...
    stmt = (
        ...
        .order_by(desc(union_stmt.c.created_at))
        .limit(limit)
    )
        ...
        result.append(
            {
                ...
                "reports": date_to_count.get(day, 0),
            }
        )
...
def get_all_users_with_stats(db: Session):
    users = db.query(User).options(joinedload(User.tier)).all()
        ...
        comment_list.append(
            {
                ...
                "post_id": c.post_id,
                "post_title": c.post.title if c.post else "(삭제된 게시글)",
            }
        )
...
def admin_delete_post(post_id: int, db: Session):
