...
const StudyMaterialsPage = () => {
  ...
  <div className="w-full min-h-screen pt-4 pl-[280px] bg-[#F9FAFB]">
    ...
    >
      {/* 검색바 + 결과 (absolute 레이어), 외부클릭 닫기용 ref 부착 */}
          ...
          {(searchQuery && searchQuery.trim().length >= 2) && (
            <div
              className="
                ...
                z-[120] rounded-xl border bg-white divide-y shadow-2xl
        ...
        <>
          <div className="relative flex justify-between items-center mb-6">
            <h1 className="text-5xl font-bold text-gray-900 mb-10">
              {studyContent.title}
              {isCompleted && (
                <span className="ml-4 text-green-600 text-xl font-semibold">✅ 학습 완료</span>
              )}
            </h1>
          ...
          <p className="text-xl text-gray-700 leading-relaxed mb-6">{studyContent.content}</p>
                              ...
                              onClick={() => {
                                ...
                                navigate(
                                  `/codetest?code=${encodeURIComponent(
                                    section.content
                                  )}&language=${encodeURIComponent(category)}&title=${encodeURIComponent(
                                    studyContent.title
