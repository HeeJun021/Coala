    ...
    return (
        <div className="p-6 bg-white shadow-lg rounded-lg border border-gray-300 w-full max-w-2xl mt-6">
            ...
            <div className="flex items-center justify-between">
                ...
                <button
                    className={`w-12 h-6 flex items-center rounded-full p-1 transition duration-300 ${notificationEnabled ? 'bg-green-500' : 'bg-gray-300'}`}
                    onClick={() => setNotificationEnabled(!notificationEnabled)}
                >
                    <div
                        className={`w-5 h-5 bg-white rounded-full shadow-md transform transition duration-300 ${notificationEnabled ? 'translate-x-6' : 'translate-x-0'}`}
                    />
                </button>
            </div>
        </div>
