    ...
    (async () => {
      try {
        ...
        const normalized = list.map((x) => ({
          ...
          project_name: x.project_name ?? "-", // 🔹 백엔드에서 내려줘야 함
          template_name: x.template_name ?? "-", // ✅ 템플릿명 사용
          ai_used: x.ai_used ?? false, // 🔹 AI 여부
          error_msg: x.error_msg ?? null, // 실패 시 메시지
        }));
        setItems(normalized);
        setTotal(cnt);
      } catch (e) {
                ...
                <table className="w-full text-sm text-gray-700 border-collapse table-fixed">
                  <thead className="bg-gray-100 border-b border-gray-200">
                    <tr>
                      ...
                      <th className="p-3 w-[10%] text-center">노션</th>
                    </tr>
                  </thead>
                  <tbody>
                    {currentItems.map((x) => (
