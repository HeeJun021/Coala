...
import {
  ...
  upsertMyPortfolioProfile as upsertPortfolioProfile,
...
function SummaryModal({
  ...
  if (!open) return null;
  const { pageTitle, targetPageTitle, templateTitle, projectName, role } =
    data || {};
  const allValid = !!(
    pageTitle?.trim() &&
    targetPageTitle &&
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              {isLoading
                ? subtitle ||
                  "잠시만 기다려 주세요. 완료되면 자동으로 상태가 바뀝니다."
                : isSuccess
...
export default function PortfolioExport() {
  ...
  const canPublish = useMemo(() => {
    return (
      !!(templateId && targetPageId && selectedProjectId && pageTitle.trim()) &&
      myLeaves >= 100
    ); // ✅ 최소 100개 필요
  ...
  if (!connected) {
    return (
      <div
        className="w-full pl-[144px] bg-[#F9FAFB] flex justify-center"
        data-scroll-root
      >
        <PortfolioGuide />
      </div>
            ...
            <div className="flex items-start justify-between">
              ...
              <div>
                ...
                <p className="text-sm text-gray-500">
                  프로젝트를 선택하고 템플릿과 대상 페이지를 정한 뒤 노션으로
                  퍼블리시하세요.
                </p>
              </div>
              ...
              {connected && (
                <div className="flex items-center gap-4 mt-1">
                  ...
                  >
                    ...
                    <span>
                      노션 연결됨{workspaceName ? ` · ${workspaceName}` : ""}
                    </span>
                  </div>
                  <button
                    type="button"
                ...
                <div className="flex flex-col gap-2">
                    ...
                    >
                      <div className="truncate text-left flex items-center gap-2">
                        ...
                        <span className="truncate">
                          {targetPageTitle || "붙여넣을 대상 페이지 선택"}
                        </span>
                      </div>
                  ...
                  <div className="flex flex-col gap-5 mt-5">
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (val && !/^010-\d{4}-\d{4}$/.test(val)) {
                              setPhoneError(
                                "전화번호 형식에 맞게 입력하세요 (예: 010-1234-5678)"
                              );
                            } else {
                              setPhoneError("");
                            }
                      ...
                      <div>
                        ...
                        <input
                          ...
                          onChange={(e) => {
                            ...
                            if (val && !/.+@.+\..+/.test(val)) {
                              setEmailError(
                                "이메일 형식에 맞게 입력하세요 (예: user@example.com)"
                              );
                            } else {
                              setEmailError("");
                            }
                    ...
                    <div className="flex flex-col gap-2 mt-5">
                      ...
                      <span className="text-[12px] text-gray-500">
                        입력하신 내용을 AI가 정리해{" "}
                        <span className="font-medium">포트폴리오 ‘경험’</span>{" "}
                        섹션에 자연스럽게 반영해요.
                      </span>
                      <textarea
                        value={aiNotes}
                        ...
                        <div className="mt-1">
                          ...
                          {result.export_history_id ? (
                            <span className="ml-2 text-gray-500">
                              (history #{result.export_history_id})
                            </span>
                          ) : null}
                        </div>
            ...
            data={{
              ...
              projectName: (() => {
                ...
                const found = (projects || []).find(
                  (p) => (p?.id ?? p?.project_id) === pid
                );
                return found?.name ?? found?.project_name ?? "";
              })(),
              role:
                (Array.isArray(filters?.my_roles) && filters.my_roles[0]) || "",
            }}
