...
import { X, Upload, MapPin, Layers, CheckCircle2, Loader2, AlertTriangle } from "lucide-react";
  ...
  const handlePublish = useCallback(async () => {
    ...
    try {
      ...
      const res = await publishToNotion({
        ...
        extra_kv: null,                   // (옵션) 추가 KV 사용 시 객체 전달
      });
      setResult(res);
      if (res?.ok) {
        alert("노션에 성공적으로 퍼블리시되었습니다.");
      } else {
        alert(res?.message || "퍼블리시에 실패했습니다.");
      }
    } catch (e) {
      console.error(e);
      setResult({ ok: false, message: e?.response?.data?.detail || e?.message || "Publish failed" });
          ...
          {!connected ? (
            ...
            <>
              ...
              <section className="rounded-2xl border border-gray-200 bg-white shadow-xl">
                ...
                <div className="p-6">
                  {/* 주의: PortfolioFilterPanel에서 다중 선택이라도, 실제 퍼블리시는 첫 번째 값만 사용 */}
                            ...
                            <div className="mt-2">
                              ...
                              <div className="mt-1 flex flex-wrap gap-2">
                                {result.missing_keys.map((k) => (
                                  <span key={k} className="text-xs bg-red-50 text-red-700 border border-red-200 rounded-full px-2 py-0.5">
                                    {k}
                                  </span>
                                ))}
                              </div>
